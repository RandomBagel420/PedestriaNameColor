...
public class DatabaseUserUtil implements UserUtil {
    ...
    public DatabaseUserUtil(@NotNull NameColor nameColor, @NotNull String mode) throws SQLException {
        ...
        try(Connection connection = dataSource.getConnection(); Statement statement = connection.createStatement()) {
            String sql = "CREATE TABLE IF NOT EXISTS namecolor_users(uuid varChar(36) primary key, nick varchar(500))";
            statement.executeUpdate(sql);

            String updateNickSize = "ALTER TABLE namecolor_users MODIFY COLUMN nick VARCHAR(500)";
            statement.executeUpdate(updateNickSize);
        } catch(SQLException e) {
            throw new SQLException("An error occurred initializing the database.");
        }
